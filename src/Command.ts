  ...
  public execute(): void {
    ...
    for (let x: number = 0; x < Canvas.WIDTH; x += Game.instance.resolution) {
      for (let y: number = 0; y < Canvas.HEIGHT; y += Game.instance.resolution) {
        ...
        let vectorFromPlayerToPoint: Vector =
          VectorMath.addVectors(playerToViewportTopLeftVector, viewportTopLeftToPointVector);
        ...
        const brightness: number =
          Math.min((GameMap.tileSize / RAW_RAY_DISTANCE[0]), 1) * Game.instance.brightnessMultiplier;
...
class UndoLastMouseMoveCommand implements Command {
  ...
  public execute(): void {
    Game.instance.player.rotatePitch(
      this.c.previousDY * Game.instance.player.rotationSpeed * Game.instance.controller.sensitivity
    );
    Game.instance.player.rotateYaw(
      -this.c.previousDX * Game.instance.player.rotationSpeed * Game.instance.controller.sensitivity
    );
  }
}
